...
public class FileTransfer extends CordovaPlugin {
    private static String resultair = "[";
    //private static JSONObject resultArr = new JSONObject[50];
        ...
        if (action.equals("upload") || action.equals("download")) {
            ...
            if (action.equals("upload")) {
                for (int i=0;i<source.length();i++){
                    upload(source.getString( i ), target, args, callbackContext,source.length());
                }
            } else {
                download(source.getString( 0 ), target, args, callbackContext);
            }
            return true;
     ...
     */
    private void upload(final String source, final String target, JSONArray args, CallbackContext callbackContext,int sourceLen) throws JSONException {
        ...
        cordova.getThreadPool().execute(new Runnable() {
            public void run() {
                ...
                try {
                    ...
                    String  res = result.toJSONObject().getString( "response" );
                    JSONObject jsonObj = new JSONObject(res);
                    JSONObject jso = jsonObj.getJSONObject( "result" ).getJSONArray( "items" ).getJSONObject( 0 );
                    resultair = resultair+jso;
                    //resultArr[count] =  jso;
                    if ((sourceLen - 1) == count){
                        resultair = resultair + "]";
                        JSONArray items = new JSONArray(resultair);
                        jsonObj.getJSONObject( "result" ).putOpt( "items",items );
                        result.setResponse( jsonObj.toString());
                        result.toJSONObject().putOpt( "response", jsonObj);
    ...
    public static void sendcontent(RequestContext context, JSONObject result, int sourceLen, int count) throws JSONException {
       ...
       if ((sourceLen - 1)== count){
           ...
           JSONArray items = new JSONArray(resultair);
           jsonObj.getJSONObject( "result" ).putOpt( "items",items );
           context.sendPluginResult(new PluginResult(PluginResult.Status.OK, jsonObj));
           count = 0;
           resultair = "[";
       }else{
           resultair = resultair + ",";
